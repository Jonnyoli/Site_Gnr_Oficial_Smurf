...
export const FINAL_STORE_ASSETS: FinalStoreAsset[] = [
  {
    "name": "Posto da Serra",
    "category": "FUNDOS",
...
export function resolveFinalStoreAsset(item: any) {
  const haystack = normalize([item?.id, item?.name, item?.collection].filter(Boolean).join(" "));

  return FINAL_STORE_ASSETS.find((asset) =>
    [asset.name, ...asset.aliases].some((alias) => haystack.includes(normalize(alias))),
  ) || null;
}

export function hasUsableStoreImage(item: any) {
  return Boolean(
    item?.image ||
    item?.imageUrl ||
    item?.previewImage ||
    ...
    item?.asset ||
    item?.assetPath ||
    resolveFinalStoreAsset(item)?.image,
  );
}
